				...
				for(int k=0; k<NoximGlobalParams::number_virtual_channel; k++)
				{
					...
					if (!buffer[i][v].IsEmpty())
					{
						...
						if (flit.flit_type==FLIT_TYPE_HEAD) 
						{
                            ...
                            NoximCoord current_coord,temp_destination_coord, destination_coord;
                            current_coord = id2Coord(route_data.current_id);
                            ...
                            }else if (flit.temp_dest_id == local_id){ // if we are elevator node
                                
                                flit.temp_dest_id = flit.dst_id;
                                
                                if (temp_destination_coord.z > current_coord.z && C_d == 1) { //we go down
                                    o = DIRECTION_DOWN;
                                }else if (temp_destination_coord.z < current_coord.z && C_u == 1){ //we go up
                                    o = DIRECTION_UP;
                                }
                            }else if (temp_destination_coord.z == current_coord.z){ // if we are on the same layer as temp address{
                                
                                if (temp_destination_coord.z > current_coord.z) { // down elevator
                                    flit.temp_dest_id = elevator_nodes_down[0];
                                }else if (temp_destination_coord.z < current_coord.z){ // up elevator
                                    
                                    flit.temp_dest_id = elevator_nodes_up[0];
                                }
                               route_data.dst_id = flit.temp_dest_id;
                               o = route(route_data, v);
                            }
                            else{ // we are middle elevator
                                if (destination_coord.z > current_coord.z && C_d == 1) { //we go down
                                    o = DIRECTION_DOWN;
                                }else if (destination_coord.z < current_coord.z && C_u == 1){ //we go up
                                    o = DIRECTION_UP;
                                }else{
                                    ...
                                    if (temp_destination_coord.z > current_coord.z) { // down elevator
                                        ...
                                        // cout<<route_data.current_id<<"  "<<flit.src_id<<"  "<<flit.dst_id<< " "<< flit.temp_dest_id<<"\n";
                                        flit.temp_dest_id = elevator_nodes_up[0];
                                    }
                                    route_data.dst_id = flit.temp_dest_id;
                                    o = route(route_data, v);
...
//                        for (int i=0 ; i< elevator_nodes_up.size(); i++)
//                            cout << elevator_nodes_up[i] << " ";
//                        cout << endl;
...
vector< int > NoximRouter::routing_ETW(NoximCoord & current, NoximCoord & destination, int dir_in)
{
    ...
    ifstream inputFile;
    inputFile.open(NoximGlobalParams::elevator_nodes_filename); // Open the file including all up and down elevators
