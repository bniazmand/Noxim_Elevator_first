...
void showHelp(char selfname[])
{
  ...
  cout << "\t\tlbdr\t\tLBDR routing" << endl;
  cout << "\t\tdqp_netz\t\tDQP-NETZ routing" << endl;
  cout << "\t\tetw\t\tETW routing" << endl;
  ...
  cout << "\t-nvirtchannel N\tSet the virtual channel number to the specified interger value (default " << DEFAULT_NUMBER_VIRTUAL_CHANNEL << "):" << endl;
  ...
  cout << "\t\tbufferlevel Based selection strategy" << endl;
  cout << "\t\tnop\t\tNeighbors-on-Path selection strategy" << endl;
  cout << "\t\tpathaware\t\tPathAware selection strategy" << endl;
  ...
  cout << "\t-volume N\tStop the simulation when either the maximum number of cycles has been reached or N KBytes have been delivered" << endl;
...
void showConfig()
{
  ...
  cout << "- max_packet_size = " << NoximGlobalParams::max_packet_size << endl;
  cout << "- routing_algorithm = " << NoximGlobalParams::routing_algorithm << endl;
  cout << "- number_virtual_channel = " << NoximGlobalParams::number_virtual_channel << endl;
  ...
  cout << "- flit_size_bits = " << NoximGlobalParams::flit_size_bits << endl;
  cout << "- number_partitions = " << NoximGlobalParams::partitions << endl;
  cout << "- data_encoding_scheme = " << NoximGlobalParams::data_encoding << endl;
  cout << "- bit_error_rate = " << NoximGlobalParams::bit_error_rate << endl;
}
...
void checkInputParameters()
{
  ...
  if (NoximGlobalParams::mesh_dim_z <= 0) {
    cerr << "Error: dimy must be greater than 0" << endl;
    exit(1);
    }
  ...
  if (NoximGlobalParams::flit_size_bits % NoximGlobalParams::partitions != 0)
  {
    cerr << "Error: the number of partitions must be an integral divisor of flit size" << endl;
    exit(1);
  }

  if (NoximGlobalParams::flit_size_bits <= 0)
  {
    cerr << "Error: flit size bits must be positive" << endl;
    exit(1);
  }

  if (NoximGlobalParams::max_volume_to_be_drained < 0)
  {
    cerr << "Error: max volume to be drained must be positive" << endl;
    exit(1);
  }

  if (NoximGlobalParams::number_virtual_channel <= 0)
  {
    cerr << "Error: virtual channel number must be positive" << endl;
    exit(1);
  }
  ...
  else
  {
	...
	char *routing = arg_vet[++i];
      ...
      NoximGlobalParams::routing_algorithm = ROUTING_LBDR;
        else if (!strcmp(routing, "dqp_netz"))
      NoximGlobalParams::routing_algorithm = ROUTING_DQP_NETZ;
        else if (!strcmp(routing, "etw"))
      NoximGlobalParams::routing_algorithm = ROUTING_ETW;
	...
	char *selection = arg_vet[++i];
      ...
      NoximGlobalParams::selection_strategy = SEL_NOP;
        else if(!strcmp(selection, "pathaware"))
      NoximGlobalParams::selection_strategy = SEL_PATHAWARE_MINIMAL;
        else 
